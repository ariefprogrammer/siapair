...
<head>
    ...
    <title>@yield('title', 'Operator') — {{ $configGeneral->app_name ?? 'SIAP AIR' }}</title>
...
<body class="bg-gray-100 min-h-screen">
                ...
                <div>
                    <p class="text-aqua text-[11px] font-semibold tracking-widest uppercase flex items-center gap-1">
                        ...
                        @else
                            <span>💧</span>
                        @endif
                        {{ $configGeneral->app_name ?? 'SIAP AIR' }} - Operator
                    </p>
    ...
    <div class="safe-bottom fixed bottom-3 left-3 right-3 z-50">
        <div class="max-w-lg mx-auto bg-deep/95 backdrop-blur text-white rounded-2xl shadow-pop px-2 py-2 flex">
            @php
                $navItems = [
                    ['route' => 'operator.dashboard', 'is' => 'operator.dashboard', 'icon' => 'fa-solid fa-house', 'label' => 'Beranda'],
                    ['route' => 'operator.catatan-meter.index', 'is' => 'operator.catatan-meter.*', 'icon' => 'fa-solid fa-file-invoice', 'label' => 'Catatan Meter'],                    
                    ['route' => 'operator.profile.index', 'is' => 'operator.profile.*', 'icon' => 'fa-solid fa-user', 'label' => 'Profile'],                    
                ];
            @endphp
    ...
    <main class="max-w-lg mx-auto px-4 pt-4 pb-24">
        ...
        @endif
        @if(session('info'))
            <div class="bg-blue-100 border border-blue-400 text-blue-800 px-4 py-3 rounded mb-4 text-sm">
                {{ session('info') }}
            </div>
        @endif
        @if($errors->any())
