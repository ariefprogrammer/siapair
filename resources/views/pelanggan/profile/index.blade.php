...
<div class="bg-white rounded-2xl shadow-sm p-4 mb-4">
    ...
    <div class="space-y-3">
        <div class="flex justify-between items-center">
            ...
            <span class="font-bold text-blue-700 font-mono">{{ $pelanggan->nomor_sambungan }}</span>
        </div>
        <div class="flex justify-between items-start">
            <span class="text-sm text-gray-500">Alamat</span>
            <span class="text-sm text-gray-800 text-right max-w-[200px]">{{ $pelanggan->alamat }}</span>
        </div>
        @if($pelanggan->rt || $pelanggan->rw)
        <div class="flex justify-between">
            <span class="text-sm text-gray-500">RT / RW</span>
        ...
        <div class="flex justify-between">
            <span class="text-sm text-gray-500">Wilayah</span>
            <span class="text-sm text-gray-800">{{ $pelanggan->wilayah }}</span>
        </div>
        @endif
        <div class="flex justify-between">
...
<div class="bg-white rounded-2xl shadow-sm p-4 mb-4">
    ...
    <form method="POST" action="{{ route('pelanggan.profile.password') }}" id="form-password">
        @csrf
                ...
                <div class="relative">
                    <input type="password"
                           ...
                           class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm pr-10
                                  focus:outline-none focus:ring-2 focus:ring-blue-400">
                    <button type="button" onclick="togglePassword('password_konfirmasi', 'eye_konfirmasi')"
                            class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">
...
document.getElementById('password_baru').addEventListener('input', function () {
    ...
    label.className   = 'text-xs ' + (score <= 1 ? 'text-red-500' : score === 2 ? 'text-orange-500' : score === 3 ? 'text-yellow-600' : 'text-green-600');
});
</script>
